...
def _resolve_patient(request):
    """Workers target an assigned patient by id; legacy patient accounts are denied."""
    user = request.user
    if user.role == "PATIENT":
        raise ApiError("Patient self-service is disabled.", code="PERMISSION_DENIED", status_code=403)
