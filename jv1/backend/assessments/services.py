...
def create_assessment(patient, data, request=None):
    ...
    if rules_out["escalate"]:
        ...
        from notifications.models import Facility
        from notifications.services import send_sms_alert

        facility = Facility.objects.filter(district=patient.district, is_ceonc=True).first()
        sms_message = f"AIMHRA alert: patient {patient.patient_code} requires {rules_out['category']} care."
        if facility:
            send_sms_alert(facility.phone_number, sms_message, related_alert=alerts[0])
        if patient.emergency_contact_number:
            send_sms_alert(
                patient.emergency_contact_number,
                "AIMHRA alert: a maternal health assessment recommends seeking medical care promptly.",
                related_alert=alerts[0],
            )
